...
export default function DashboardFeedbackError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <section className="dashboard-content">
      <div className="dashboard-state dashboard-error" style={{ minHeight: '40vh' }}>
        ...
        <strong style={{ color: '#10233f', fontSize: 16 }}>Impossible de charger les témoignages</strong>
        <p style={{ margin: 0, color: '#7692a9', fontSize: 12, lineHeight: 1.6, maxWidth: 380 }}>
          {error?.digest ? `Référence : ${error.digest}` : 'Le service témoignages est temporairement indisponible.'}
        </p>
