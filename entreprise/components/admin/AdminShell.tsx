...
export default function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  async function logout() { try { await fetch('/api/auth/cookie', { method: 'DELETE', credentials: 'include' }); } catch {} try { localStorage.removeItem('jobsinc_token'); localStorage.removeItem('jobsinc_admin_user'); } catch {} document.cookie = 'jobsinc_token=; path=/; max-age=0'; document.cookie = 'jobsinc_admin_user=; path=/; max-age=0'; // Déconnexion côté backend (clear httpOnly backend cookie)
